...
*2026-07-06. Status: **APPROVED FOR PROMOTION** (nrd, 2026-07-07 — core
...
older documents accordingly.*

*v0.2 (2026-07-07): incorporates all ten corrections from a zero-context
...
the μ/ν and monoidal-category decorations struck (§4.4, §5.1).*

*v0.3 (2026-07-07): extensions from nrd's expansion — interfaces as the
...
layer's own operation, proving install needs zero new concepts (§6.4).*

*v0.4 (2026-07-07): the language-free purity thesis promoted to §0.5 (the
...
gap; §9.11 rewritten accordingly.*

*v0.5 (2026-07-07): the two strata of intent (§1.5) — executable intent
...
certificate identified as the environment's lockfile.*

*v0.6 (2026-07-07): §3.5 — promotion as reified monadic bind (the IFD
...
*v0.7 (2026-07-07): corrections from the SECOND zero-context adversarial
...
in §9.14 as the lock campaign's acceptance criteria.*

*v0.8 (2026-07-07): whole-system corrections from nrd — the model was
...
discipline — what this model deliberately does not own.*

*Trigger: early substrate implementation work treated **build** as the
...
derive as policy strata — rather than as separately engineered features.*
...
> stratified *by the request's policy* — never by workload kind — into
> *actions* (all ambient channels discharged, hence cacheable) and *trials*
...
language's semantics is what guarantees that package *composition*
...
> — a *provenance* claim, which evaporates the moment an object crosses a
...
> *is* its meaning; substitution of interface-equals is the β-rule, §1.4),
...
> *by the object*, which is the only kind of purity that survives
> decentralization.
...
Nix closure *why* it contains a path; it is the residue of an arbitrary
Turing-complete evaluation. You can ask a certificate). In *Build Systems
à la Carte* terms: the model deliberately occupies the **applicative
...
The generating *language* thereby becomes optional, untrusted sugar:
...
- **Composition** `c` — a finite map from conventional paths to content
  entries, signed. Its *denotation* is a partial function:
...
Disagreement is a *conflict* — merge is undefined, surfaced as an explicit
...
*denotations* disagree on any path in the intersection of their
denotation domains (equivalently: mandate a prefix-free normal form and
check keys there). *Proof obligation P1 (cheap, Alloy-able): the concrete
merge implementation satisfies the laws AND detects exactly the
denotational conflicts — the graft case is in P1's scope explicitly.*

### 1.3 The world and its channels

Everything a process can be influenced by that is *not* in its view is an
...
The merge monoid (§1.2) says when compositions *can* coexist (path
disjointness). It says nothing about whether the result *works*. That is
...
contract"** — normalized, hashed, unforgeable, *decidable by
construction* where semantic ABI equality is not. (Honesty is inherited
...
by *composing* the missing provider into the environment (or declaring a
...
**Versioning.** An environment's *identity* is its composition root; its
*contract* is its interface (Prov/residual Req and their digests). A
semantic version is neither — it is human-facing naming, and it applies
exactly when an environment is *published as an atom* (signed intent
...
mapping that gives layering *meaning* — the layer boundary is a certified
...
cuts at the right joint only when asked about *laws*, not *schemas* — and
...
constraints) — what environments lack is not dependency *declaration* but
build-input *semantics* (no toolchain, no fetch set, no build/runtime dep
split).
...
*steps*; an atom denotes an elaboration *pipeline*, and pipelines may
...
are executions *of* the finished artifact at use time, outside elaboration
...
execution records and attestations as *inputs* ("admit P only if its
...
- **Request completeness.** Every pinned channel carries a *payload* (the
  ...
  defect class (the seed enters via `view`, so it *is* digested, but the
  lesson generalizes: anything that can influence the record must be
  reachable from `req_digest`). *Proof obligation P5: an
  injectivity/completeness audit of the canonical request serialization —
  every field the executor consults appears in the digest preimage.*
...
the following true *by construction* rather than by trust.

**The semantics is relational, not functional — deliberately.** Even with
every ambient channel closed or pinned, *residual nondeterminism* remains
...
discharged; what is *not* claimed is that the set is a singleton. The
...
the per-channel discharge — a *conformance checklist for executor
implementations*, not a metaphysical claim.
...
hashes"). A *cache hit* is: ∃ a witness acceptable under the consumer's
...
request is formed, the witness *pick* is a **recorded choice over the
...
- An **action**'s record is a **fact**: a signed witness of `⟦req⟧`,
  ...
  payload.** For a hermetically *flaky* test (racy threads), first-
...
- A **trial**'s record is an **attestation**: a signed *witness* that "at
  ...
  *accepted* in lieu of re-running is an explicit acceptance policy
...
- **Reproducible**: two *executions* of the same action yield byte-identical
  ...
  not. Cache correctness needs only hermeticity; *trust* policies (who may
...
manifest→lock, and `resolve` is not even *total* from lock data today:
...
rebuilder of *Build Systems à la Carte*, obtained structurally. (À la carte
...
executors *by construction*, and stdio is not bit-stable even for
...
conformance (§6.1) all mean *core*-equality. (Correction F4, second
adversarial review: v0.6 let three inequivalent equalities drift across
those sections.)

Note what is *absent*: `BuildResult` carrying a read-set (Gate 0's design)
...
A build fact is *advertised* (published into the fact channel — appended
...
test never forces a rebuild) — only its *advertisement* is gated. Gating is
...
tool then **promotes** that attestation into *intent*: fetch-set entries
...
epistemics as a Nix FOD hash bump — formalized as the *only* sanctioned way
...
already reviewed, pinned intent — and it lives *inside the atom's
sources*, hence inside the closure, hence inside `action_id`, so request
...
This is a *decentralized* stack: records arrive signed by executors you do
not control. Hermeticity is a property of *honest* execution — it does
nothing to make a remote builder honest. So **fact-hood is relative to a
trust anchor set**: a record is accepted *as a fact* (cache-usable,
...
**the action/trial stratification governs what a record *can* be; signer
trust governs what it *is, to you*.** The concrete anchor format and
...
In *Build Systems à la Carte* terms, dynamic dependency — computing the
next build from a previous build's *result* — is monadic `bind`, and
...
> into signed intent; monadic builds are realized as an *iteration of
> applicative generations* with signatures at the join points. Each
...
obligations, P6, not as assumed structure): *left identity* — promoting
an already-known fact is equivalent to having declared it as intent;
*idempotence* — re-promoting without change is a no-op; *confluence of
independent promotions* — concurrent promotions touching disjoint intent
...
Everything below is about maintaining *certified bounds* — and the two
bounds quantify over *different* dependency sets, which is why the index
matters.
...
- **Upper bound — structural, free, and per-view.** The namespace bounds
  ...
  its view, so `Dep_v(a) ⊆ dom(⟦v⟧)` holds *by construction*, for each
  view separately — no tracing, no checking, no FUSE required. What does
  **not** hold is `Dep(a) ⊆ dom(⟦v⟧)` for any particular `v`: an
  execution in a *larger* view can read paths this view lacks. That is
  ...
  execution ever touches) are *real* in this model. Nix never had to say
...
- **`Declared` — the human escape hatch.** Because `Dep(a)` is
  uncomputable, detection *will* miss; a human may declare a runtime
  ...
  may be wrong in the fat direction, and that is *safe* (an unused
...
*Proof obligation P2 (real, small, Lean-able): `J` is monotone in its
...
`Dep(a)`, never oscillates.* Every entry of `J` carries a **justification
object** — a binding proof or an observation record — so the closure is not
merely computed but *explainable*, entry by entry (contrast: Nix's closure
...
sound *with respect to every behavior witnessable within it* — shipping
...
read outside `J` is a **closure fault**: denied, logged *with the name that
missed*. The fault is itself an observation — a new justification candidate
...
inside the sound upper bound. Formally this is *one* mechanism, applied
...
in *where their generating sets come from* (declaration vs. observation),
...
- **Track A's overlap-safety argument transfers after a wording repair,
  ...
  denies. The *correct* justification needs no determinism: witnesses
  ...
  one BuildRecord per action" → one per *build event* (accumulating).
...
Tests (of either stratum) do not appear in the *build* DAG: no build
...
blobs referenced *by the attestation*, never view-entering outputs.

**Theorem (no-rebuild-on-test-edit).** Test parameters occur only in test
requests (of either stratum). `action_id` and every *build* `req_digest`
...
entry, and triggers no rebuild — *structurally*, not by scheduler courtesy;
...
Its validity is *checked, per consumer*: for each require previously bound
...
- **Rebinding (runtime stratum, cheap).** Consumers whose interface
  contracts survive the swap are *relinked*: composition edit, recertify,
...
- **Rebuild frontier (intent stratum, bounded).** Consumers where
  satisfaction *fails* form the frontier `F` — they genuinely need their
...
*where `F` is computed against `iface_digest` satisfaction* — and per
...
being replaced: Nix rebuilds the *entire reverse-dependency closure* of
...
diffing; a generator here is *statically diffable* as a certificate
...
into one derivation makes the *pair* the unit of caching, so a sandbox-
...
(test-flag edit, network policy change) re-runs *only the trial*.
...
- on **actions**: every record either produces lies in `⟦req⟧` — i.e.
  ...
  semantics: conforming executors may return *different witnesses* of a
  non-singleton set, and the cache's first-witness fiat absorbs that). The
  *testable* form: on requests empirically known reproducible (singleton
  `⟦req⟧` attested per §2.3), conforming executors MUST produce
  digest-identical records — golden-request conformance uses exactly
  these;
- on **trials**: they produce *valid* attestations for identical requests
  (agreement up to the policy's licensed nondeterminism — a test may pass
  on one run and fail on another *only* through open channels or residual
  nondeterminism).
...
- Progress/duality for `ExecuteSession` is *proof obligation P3* (cheap,
...
| nproc / CPU count | — | pinned via cgroup cpuset or params | **unpinned today**: `make -j$(nproc)` reads it (Gate A/B literally did); affects parallelism (usually not bytes, but *usually* is not a proof) |
...
requires only *soundness* of observed reads (every logged read really
happened); instruments differ in *coverage*, and coverage is an empirical
...
a second — so an observed read-set from ptrace is a lower bound *relative
to its trapped surface*. FUSE had the complementary profile: structurally
...
fs-verity: the kernel refuses corrupted content *at read time* — a
...
§5.2: fetch-by-digest is *substitution into an existing binding*, the
...
stringly platform properties ↔ our policy classification (lossy — *the policy
semantics is precisely our novelty*), their unsigned actions ↔ our signed
...
| htc-sad §2.3 `BuildRecord.observed_read_set_digest` | read-set as a standing BuildRecord field | same defect class as the row above, at the *doctrinal* home (F9, second review — v0.6's sweep missed it): becomes optional/absent for untraced executions; populated only when policy.observe was on |
...
root: reviewers (and the formalizer) importing obligations the *system*
...
- **P4** *(re-scoped in v0.2)* — a small state-machine specification of
  ...
  *specification* of the executor, not as mathematical news.
- **P5** *(added in v0.2)* — request-serialization completeness (§2.1):
  ...
  more than a theorem.
- **P6** *(added in v0.6; re-scoped in v0.8 per nrd)* — promotion
  ...
  **reorder-invariance of independent sequential promotions** (running
  ...
  an overcorrection: the system's write topology is *phased by design*
...
- **P7** *(added in v0.7, from F3; extended v0.8)* — `resolve` is a
...
- **P4 (extended v0.7; shrunk v0.8)** — covers concurrent completion as
  *set-accumulation*: check that commutative witness insertion preserves
...
3. **Trial acceptance policy.** *(Narrowed in v0.2: hermetic tests cache
   as actions — no policy needed there.)* For genuine trials (networked
...
5. ~~Trial output scope~~ — *resolved in v0.2 by the deposit/bind law
   (§5.2): trials deposit CAS values referenced by their attestations;
   binding into views happens only via promotion through signed intent.*
...
7. **Trust-anchor policy language** *(added in v0.2, from §3.4)*: fact-hood
...
8. **The ambient base** *(added in v0.3, from §1.4)*: an environment's
...
9. **Coherence choice function defaults** *(added in v0.3)*: "supported
...
10. **Environments as atoms** *(added in v0.3)*: publishing an
    ...
    lean is yes, but it makes environments *publishable intent*, which
    deserves a deliberate decision rather than a default.
11. **The override operator set** *(rewritten in v0.4 — v0.3 misfiled
    this as a "generator gap")*: overlays are not a missing language
    ...
    is only the *surface*: the canonical operator set, whether overlays
...
12. **Env vars as an interface namespace** *(added in v0.4, from the
    generator audit)*: dev shells and some runtimes need
...
13. **Atom kind schema policy** *(added in v0.5, from §1.5; narrowed same
    day)*: strata are closed and law-bearing; kinds are open and
    schema-level. The mixed-atom question is *answered in the model* —
    ...
    their legality.
14. **The lock format** *(added v0.6; re-assessed v0.8 after nrd's
    whole-system corrections)*: the lock is binding-algebra instantiation
    #1 and the promotion target (§3.3); its formal treatment is an
    operational TOML spec, and *some* elaboration is owed — but less
