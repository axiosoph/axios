...
3. **L3: Eos** (`eos/`) — The atom-DAG build scheduler. Scheduler traits (`eos-core`) exist; the `eos-snix` build engine is pre-substrate code slated for removal — the evaluator is deleted from the design (ADR-0006) — with the executor re-cut around HTC's trait pending. The daemon (`eos-daemon`) and Cap'n Proto RPC protocol (`eos-proto`) are under development.
