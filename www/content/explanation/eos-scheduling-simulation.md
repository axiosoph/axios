...
> **Re-scope note:** This study measured coarsening at Nix derivation granularity. Eos now schedules over a pre-coarsened atom-DAG (package-scale nodes read directly off the lock, per [ADR-0005](../architecture/0005-hermetic-transactional-composition.md)), one level coarser than the graph this study traced. The results below are historical evidence for the directional finding — coarsening choice matters, and matters more as parallelism grows — not a claim about atom-DAG-granularity behavior; revalidation at that granularity is future work.
...
The quantitative gaps (0.28%, 0.43%) are specific to this graph family and its estimated build durations. The directional rankings are structural and were expected to hold across nixpkgs versions because the deep bootstrap topology this study traced was a stable property of how nixpkgs packages build at derivation granularity — but that premise is exactly what the atom-DAG re-scope changes: Eos no longer sees that deep topology at all, since the graph it schedules over arrives pre-coarsened at package scale before Eos ever touches it. This does not falsify the directional rankings — package-scale coarsening choices plausibly matter for similar reasons — but the specific mechanism this study measured (heuristics operating over a deep, multi-thousand-node bootstrap graph) no longer describes what Eos sees, and a separate study at atom-DAG granularity would be needed to make the claim there directly.

The simulation also does not model cross-worker artifact transfer costs explicitly. The coarsening heuristics partially address this by design: promoting high-fan-in shared nodes to entry points — exactly the nodes whose outputs are most widely consumed — allows PEFT to co-locate downstream EPs via cache affinity routing, which is the primary mitigation. The secondary mitigation is the underlying CAS's intra-file chunk-level deduplication (built on `snix-castore`): transfer cost is proportional to novel chunk content, not output archive size, and declines as the store fills from prior builds. The combination of affinity-aware dispatch and CAS-level deduplication substantially reduces the residual transfer exposure. The gap that remains — large-output, low-fan-in nodes not on the critical path — is unlikely to reverse the directional heuristic rankings, though it could affect quantitative margins on heterogeneous networks. A full accounting would require artifact size data and a CAS dedup ratio model not currently in the corpus.
