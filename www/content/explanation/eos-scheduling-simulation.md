...
The simulation also does not model cross-worker artifact transfer costs explicitly. The coarsening heuristics partially address this by design: promoting high-fan-in shared nodes to entry points — exactly the nodes whose outputs are most widely consumed — allows PEFT to co-locate downstream EPs via cache affinity routing, which is the primary mitigation. The secondary mitigation is Snix's intra-file chunk-level deduplication: transfer cost is proportional to novel chunk content, not output archive size, and declines as the blobstore fills from prior builds. The combination of affinity-aware dispatch and Snix deduplication substantially reduces the residual transfer exposure. The gap that remains — large-output, low-fan-in nodes not on the critical path — is unlikely to reverse the directional heuristic rankings, though it could affect quantitative margins on heterogeneous networks. A full accounting would require artifact size data and a Snix dedup ratio model not currently in the corpus.
