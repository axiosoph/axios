...
The atom lock file — a per-project manifest of resolved atom versions — is a coarser representation of the same dependency structure the derivation DAG captures. When multiple requests arrive concurrently, their lock files will typically share substantial overlap: the same atoms at the same versions, producing identical derivation subgraphs. An evaluation scheduler that exploits this — grouping concurrent requests by lock-file similarity to evaluate overlapping atoms once — could meaningfully reduce redundant evaluation work, and it compounds: by producing a single pre-merged derivation graph rather than N separate ones, the eval scheduler also reduces the build scheduler's deduplication burden. Jobs submitted concurrently already coalesce in the build scheduler into a unified global DAG, but that merging happens after evaluation. An eval scheduler that pre-coalesces overlapping derivations hands the build scheduler a richer, already-unified graph — it begins with a complete picture of shared structure rather than reconstructing it across independent inputs.
