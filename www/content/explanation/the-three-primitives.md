...
- **Swap with a bounded blast radius.** Replacing a library (say, a patched OpenSSL) is a checked edit: for every consumer whose declared interface still matches, the composition simply points at the new library — a metadata change, no linker runs, no binary is touched — and the dynamic loader finds the new version at startup. Only the consumers the check _fails_ for actually rebuild. Nix, by contrast, must rebuild everything downstream, because it has no way to check that anything survived.
