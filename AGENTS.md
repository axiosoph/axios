...
>
> This table is an agent-facing **derivative** of the canonical
> [project glossary](docs/glossary.md). On any conflict, the glossary wins
> and this table is corrected to match — never the reverse.

| Canonical Term         | Definition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Deprecated Aliases (NEVER use)    |
| :--------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------- |
| **Anchor**             | The mechanism linking a scope to structure — a role, not a value — in two families: source anchors (a record’s position; the genesis commit pre-signing) and authority anchors (set anchor = the charter, label anchor = the claim). The chartered set anchor is referenced by the charter’s czd; the czd names the anchor, it is not the anchor. See [docs/glossary.md](docs/glossary.md).                                                                                                                                                    | genesis, root, Root               |
| **Atom identity**      | The claim record's czd, for any claimed atom. The label is a declared field of the claim; the anchor is committed to through the record's signed chain links, which walk back to the charter genesis — never a trusted declared field. Before a claim exists (an unclaimed dev atom), identity is a digest of anchor and label as preimage — the pair's only role anywhere. `AtomId` as a Rust type name is code convention, not doctrine. Lock entries are `(set, label) → {version, publish_czd}`. See [docs/glossary.md](docs/glossary.md). | hashed atom id, atom_id-as-digest |
